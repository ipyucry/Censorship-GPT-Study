...
def calculate_average_sentiment(df):
    avg_self_sentiment = df[df['category'] == 'U.S.']['score_avg'].mean()
    avg_other_sentiment = df[df['category'] == 'Russia']['score_avg'].mean()
    return avg_self_sentiment, avg_other_sentiment

# performing a t-test between self and other categories
def perform_t_test(df):
    self_scores = df[df['category'] == 'U.S.']['score_avg'].dropna()
    other_scores = df[df['category'] == 'Russia']['score_avg'].dropna()
...
def plot_sentiment_difference(df, avg_self, avg_other):
    ...
    plt.axhline(avg_self, color='blue', linestyle='--', label=f'U.S. Avg: {avg_self:.2f}')
    plt.axhline(avg_other, color='red', linestyle='--', label=f'Russia Avg: {avg_other:.2f}')

    # adjusting labels and title for compactness
    plt.title('English GPT', fontsize=10)
...
def check_for_bias(file_path):
    ...
    print(f'Average sentiment for U.S.: {avg_self}')
    print(f'Average sentiment for Russia: {avg_other}')
...
csv_file_path = '../results/cgpt4o_political_prompts_rus_eng_scored.csv'
